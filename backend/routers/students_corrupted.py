...
Handles student registration,         logger.info(f"Create class error: {e}", exc_info=True)
...
async def delete_class(
    ...
    try:
        ...
        if len(class_obj.students) > 0:
            raise HTTPException(
                status_code=400, 
                detail=f"Cannot delete class with {len(class_obj.students)} students. Please remove all students first."
            )
...
@router.get("/backups")ement, and attendance operations.
"""
import os
import shutil
import logging
